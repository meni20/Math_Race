...
import { leaveRoom } from "../_shared/game-core.ts";
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const result = await runRoomMutation(
      ...
      now,
      (room, presenceByPlayerId) => leaveRoom(room, normalizedPayload, presenceByPlayerId, now)
    );
    return jsonResponse(result.response);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unexpected error";
    return jsonResponse({ error: buildError("LEAVE_FAILED", message) }, 400);
  }
});
