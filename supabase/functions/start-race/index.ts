import type { StartRaceRequest } from "../_shared/contracts.ts";
import { createAdminClient } from "../_shared/admin.ts";
import { startRace } from "../_shared/game-core.ts";
...
Deno.serve(async (request) => {
  ...
  try {
    const payload = await readJsonRequest<StartRaceRequest>(request);
    const normalizedPayload: StartRaceRequest = {
    ...
    const result = await runRoomMutation(
      ...
      now,
      (room, presenceByPlayerId) => startRace(room, normalizedPayload, presenceByPlayerId, now)
    );
    return jsonResponse(result.response);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unexpected error";
    return jsonResponse({ error: buildError("START_RACE_FAILED", message) }, 400);
  }
});
