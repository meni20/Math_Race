...
import { insertRoomEvent, markRoomParticipantKicked } from "../_shared/classroom-store.ts";
import { teacherRemovePlayer } from "../_shared/game-core.ts";
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const result = await runRoomMutation(
      ...
      now,
      (room, presenceByPlayerId) => teacherRemovePlayer(room, normalizedPayload, presenceByPlayerId, now)
    );
    await markRoomParticipantKicked(admin, normalizedPayload.roomId, normalizedPayload.targetPlayerId, now);
    await insertRoomEvent(admin, normalizedPayload.roomId, "PARTICIPANT_REMOVED", {
      playerId: normalizedPayload.targetPlayerId
    });
    return jsonResponse(result.response);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unexpected error";
    return jsonResponse({ error: buildError("TEACHER_REMOVE_FAILED", message) }, 400);
  }
});
