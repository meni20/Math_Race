...
Deno.serve(async (request) => {
  ...
  try {
    const payload = await readJsonRequest<JoinGameRequest>(request);
    const normalizedPayload: JoinGameRequest = {
    ...
    const result = await runRoomMutation(
      ...
      now,
      (room) => joinRoom(room, normalizedPayload, now)
    );
    return jsonResponse(result.response);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unexpected error";
    return jsonResponse({ error: buildError("JOIN_FAILED", message) }, 400);
  }
});
