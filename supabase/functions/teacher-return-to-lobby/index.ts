...
import { teacherReturnToLobby } from "../_shared/game-core.ts";
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const normalizedPayload: TeacherRoomRequest = {
    ...
    const result = await runRoomMutation(
      ...
      now,
      (room, presenceByPlayerId) => teacherReturnToLobby(room, normalizedPayload, presenceByPlayerId, now)
    );
    return jsonResponse(result.response);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unexpected error";
    return jsonResponse({ error: buildError("TEACHER_RETURN_FAILED", message) }, 400);
  }
});
