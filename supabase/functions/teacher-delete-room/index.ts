...
import { markClassroomRoomDeleted } from "../_shared/classroom-store.ts";
import { teacherDeleteRoom } from "../_shared/game-core.ts";
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const result = await runRoomMutation(
      ...
      now,
      (room, presenceByPlayerId) => teacherDeleteRoom(room, normalizedPayload, presenceByPlayerId, now)
    );
    await markClassroomRoomDeleted(admin, normalizedPayload.roomId, normalizedPayload.teacherSessionId, now);
    return jsonResponse(result.response);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unexpected error";
    return jsonResponse({ error: buildError("TEACHER_DELETE_FAILED", message) }, 400);
  }
});
