import type { SyncRoomRequest } from "../_shared/contracts.ts";
import { createAdminClient } from "../_shared/admin.ts";
import { returnPlayerToLobby } from "../_shared/game-core.ts";
...
Deno.serve(async (request) => {
  ...
  try {
    const payload = await readJsonRequest<SyncRoomRequest>(request);
    const normalizedPayload: SyncRoomRequest = {
    ...
    const result = await runRoomMutation(
      ...
      now,
      (room, presenceByPlayerId) => returnPlayerToLobby(room, normalizedPayload, presenceByPlayerId, now)
    );
    return jsonResponse(result.response);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unexpected error";
    return jsonResponse({ error: buildError("RETURN_TO_LOBBY_FAILED", message) }, 400);
  }
});
